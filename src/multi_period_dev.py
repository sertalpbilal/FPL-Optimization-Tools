...
def prep_data(my_data, options):
    ...
    locked_next_gw = [
        i if isinstance(i, int) else int(i[0])
...
def solve_multi_period_fpl(data, options):
    ...
    model.add_constraints(
        (
            so.expr_sum(squad[p, w] for p in players if merged_data.loc[p, "name"] == t)
            <= 3
            for t in teams
            for w in gameweeks
        ),
        ...
        model.add_constraints(
            (
                so.expr_sum(
                    squad[p, w]
                    for p in players
                    if merged_data.loc[p, "name"] == t
                    and merged_data.loc[p, "Pos"] in {"G", "D"}
                )
                <= max_defs_per_team
                for t in teams
                for w in gameweeks
        ...
        for gw in gameweeks:
            ...
            if options.get("opposing_play_group", "all") == "all":
                opposing_players = [
                    ...
                    for f in gw_games
                    for p1 in players
                    if merged_data.loc[p1, "name"] == f["home"]
                    for p2 in players
                    if merged_data.loc[p2, "name"] == f["away"]
                ]
                model.add_constraints(
                    (
