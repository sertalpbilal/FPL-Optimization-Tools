...
def xmin_to_prob(xmin, sub_on=0.5, sub_off=0.3):
    start = min(
        max(
            (xmin - 25 * sub_on) / (90 * (1 - sub_off) + 65 * sub_off - 25 * sub_on),
            0.001,
        ),
        0.999,
    )
...
def get_dict_combinations(my_dict):
    ...
    for key in keys:
        if my_dict[key] is None or len(my_dict[key]) == 0:
            my_dict[key] = [None]
    all_combs = [
        dict(zip(my_dict.keys(), values)) for values in product(*my_dict.values())
    ]
    feasible_combs = []
...
def connect():
    print(
        "This method is provided for people who want to automate login. However with 2FA changes, it is unlikely to work, and I'd suggest using team.json method from now on as explained in README file."
    )
    base_folder = Path()
...
def generate_team_json(team_id):
    ...
    for player_id, purchase_price in squad.items():
        now_cost = [x for x in static["elements"] if x["id"] == player_id][0][
            "now_cost"
        ]
...
def prep_data(my_data, options):
    ...
    locked_next_gw = [
        int(i[0]) if isinstance(i, list) else int(i)
        for i in options.get("locked_next_gw", [])
    ]
    ...
    for pos, vals in options.get("pick_prices", {}).items():
        ...
        pp = merged_data[
            (merged_data["Pos"] == pos)
            & ((merged_data["now_cost"] / 10).isin(price_vals))
        ]["review_id"].to_list()
    ...
    merged_data = merged_data[
        (merged_data["total_min"] >= xmin_lb)
        | (merged_data["review_id"].isin(safe_players))
    ].copy()
    ...
    if ev_per_price_cutoff != 0:
        cutoff = (merged_data["total_ev"] / merged_data["now_cost"]).quantile(
            ev_per_price_cutoff / 100
        )
        merged_data = merged_data[
            (merged_data["total_ev"] / merged_data["now_cost"] > cutoff)
            | (merged_data["review_id"].isin(safe_players))
        ].copy()
    ...
    if options.get("randomized", False):
        ...
        for w in gws:
            noise = (
                merged_data[f"{w}_Pts"]
                * (92 - merged_data[f"{w}_xMins"])
                / 134
                * rng.standard_normal(size=len(merged_data))
            )
    ...
    if not preseason:
        for i in my_data["picks"]:
            if buy_price[i["element"]] != sell_price[i["element"]]:
                ...
                print(
                    f"Added player {i['element']} to list, buy price {buy_price[i['element']]} sell price {sell_price[i['element']]}"
                )
...
def solve_multi_period_fpl(data, options):
    ...
    squad_fh = model.add_variables(
        players, gameweeks, name="squad_fh", vartype=so.binary
    )
    ...
    bench = model.add_variables(
        players, gameweeks, order, name="bench", vartype=so.binary
    )
    ...
    transfer_out_first = model.add_variables(
        price_modified_players, gameweeks, name="tr_out_first", vartype=so.binary
    )
    transfer_out_regular = model.add_variables(
        players, gameweeks, name="tr_out_reg", vartype=so.binary
    )
    transfer_out = {
    ...
    transfer_count = model.add_variables(
        gameweeks, name="trc", vartype=so.integer, lb=0, ub=15
    )
    ...
    if run_chip_combinations is not None:
        ...
        if len(chip_combinations) > 0:
            if set(chip_combinations[0].values()) == {None}:
                ...
                print(
                    "You have active chip combinations, iteration parameter will be overridden"
                )
                ...
                for pair in pairs:
                    ...
                    if current_chips.get(chip) is not None:
                        model.add_constraint(
                            variable[current_chips[chip]] == 1, name=f"cc_{chip}"
                        )
                        options["chip_limits"][chip] = 1
                    else:
                        model.add_constraint(
                            so.expr_sum(variable[w] for w in gameweeks) == 0,
                            name=f"cc_{chip}",
                        )
    ...
    model.add_constraints(
        (squad[p, next_gw - 1] == 0 for p in players if p not in initial_squad),
    ...
    model.add_constraints(
        (squad_fh_count[w] == 15 * use_fh[w] for w in gameweeks), name="squad_fh_count"
    )
    model.add_constraints(
        (
            so.expr_sum(lineup[p, w] for p in players) == 11 + 4 * use_bb[w]
            for w in gameweeks
        ),
        name="lineup_count",
    )
    model.add_constraints(
        (
            so.expr_sum(bench[p, w, 0] for p in players if player_type[p] == 1)
            == 1 - use_bb[w]
            for w in gameweeks
        ),
        name="bench_gk",
    ...
    model.add_constraints(
        (
            so.expr_sum(bench[p, w, o] for p in players) == 1 - use_bb[w]
            for w in gameweeks
            for o in [1, 2, 3]
        ),
        name="bench_count",
    ...
    model.add_constraints(
        (lineup[p, w] <= squad[p, w] + use_fh[w] for p in players for w in gameweeks),
    ...
    model.add_constraints(
        (
            lineup[p, w] + so.expr_sum(bench[p, w, o] for o in order) <= 1
            for p in players
            for w in gameweeks
        ),
    ...
    model.add_constraints(
        (
            lineup_type_count[t, w] <= type_data.loc[t, "squad_max_play"] + use_bb[w]
            for t in element_types
            for w in gameweeks
        ),
    ...
    model.add_constraints(
        (
            squad_fh_type_count[t, w] == type_data.loc[t, "squad_select"] * use_fh[w]
            for t in element_types
            for w in gameweeks
        ),
        name="valid_squad_fh",
    ...
    model.add_constraints(
        (
            so.expr_sum(squad[p, w] for p in players if player_team[p] == t) <= 3
            for t in teams
            for w in gameweeks
        ),
        name="team_limit",
    ...
    model.add_constraints(
        (
            so.expr_sum(squad_fh[p, w] for p in players if player_team[p] == t)
            <= 3 * use_fh[w]
            for t in teams
            for w in gameweeks
        ),
    ...
    model.add_constraints(
        (
            ...
            + sold_amount[w]
            - bought_amount[w]
            - (transfer_count[w] * itb_loss_per_transfer if w > next_gw else 0)
            for w in gameweeks
        ),
        name="cont_budget",
    )
    model.add_constraints(
        (
            so.expr_sum(fh_sell_price[p] * squad[p, w - 1] for p in players)
            + in_the_bank[w - 1]
    ...
    model.add_constraints(
        (transfer_in[p, w] <= 1 - use_fh[w] for p in players for w in gameweeks),
        name="no_tr_in_fh",
    )
    model.add_constraints(
    ...
    raw_gw_ft = {
        w: free_transfers[w] - transfer_count[w] + 1 - use_wc[w] - use_fh[w]
        for w in gameweeks
    }
    model.add_constraints(
        (
    ...
    model.add_constraints(
        (aux[w] <= 1 - use_wc[w - 1] for w in gameweeks if w > next_gw),
        name="ft_after_wc",
    )
    model.add_constraints(
        (aux[w] <= 1 - use_fh[w - 1] for w in gameweeks if w > next_gw),
        name="ft_after_fh",
    )
    ...
    model.add_constraint(
        so.expr_sum(use_wc[w] for w in gameweeks) <= chip_limits.get("wc", 0),
        name="use_wc_limit",
    )
    model.add_constraint(
        so.expr_sum(use_bb[w] for w in gameweeks) <= chip_limits.get("bb", 0),
        name="use_bb_limit",
    )
    model.add_constraint(
        so.expr_sum(use_fh[w] for w in gameweeks) <= chip_limits.get("fh", 0),
        name="use_fh_limit",
    )
    model.add_constraint(
        so.expr_sum(use_tc_gw[w] for w in gameweeks) <= chip_limits.get("tc", 0),
        name="use_tc_limit",
    )

    model.add_constraints(
        (squad_fh[p, w] <= use_fh[w] for p in players for w in gameweeks),
        name="fh_squad_logic",
    )
    ...
    model.add_constraints(
        (
            horizon
            * so.expr_sum(transfer_out_first[p, w] for w in gameweeks if w <= wbar)
    ...
    model.add_constraints(
        (
            so.expr_sum(transfer_out_first[p, w] for w in gameweeks) <= 1
            for p in price_modified_players
        ),
        name="multi_sell_3",
    )
    ...
    model.add_constraints(
        (
            transfer_count[w] >= number_of_transfers[w] - 15 * use_wc[w]
            for w in gameweeks
        ),
        name="trc_lb",
    )
    model.add_constraints(
        (transfer_count[w] <= number_of_transfers[w] for w in gameweeks), name="trc_ub1"
    )
    model.add_constraints(
        (transfer_count[w] <= 15 * (1 - use_wc[w]) for w in gameweeks), name="trc_ub2"
    )
    ...
    if options.get("banned", None) is not None:
        ...
        model.add_constraints(
            (so.expr_sum(squad[p, w] for w in gameweeks) == 0 for p in banned_players),
            name="ban_player",
        )
        model.add_constraints(
            (
                so.expr_sum(squad_fh[p, w] for w in gameweeks) == 0
    ...
    if options.get("no_future_transfer"):
        model.add_constraint(
            so.expr_sum(
                ...
                for p in players
                for w in gameweeks
                if w > next_gw and w != options.get("use_wc")
            )
            == 0,
    ...
    if options.get("no_transfer_last_gws"):
        ...
        if horizon > no_tr_gws:
            model.add_constraints(
                (
                    so.expr_sum(transfer_in[p, w] for p in players) <= 15 * use_wc[w]
                    for w in gameweeks
                    if w > last_gw - no_tr_gws
                ),
                name="tr_ban_gws",
            )
    ...
    if options.get("num_transfers", None) is not None:
        model.add_constraint(
            so.expr_sum(transfer_in[p, next_gw] for p in players)
    ...
    if options.get("hit_limit", None) is not None:
        model.add_constraint(
            so.expr_sum(penalized_transfers[w] for w in gameweeks)
            <= int(options["hit_limit"]),
    ...
    if options.get("weekly_hit_limit") is not None:
        ...
        model.add_constraints(
            (penalized_transfers[w] <= weekly_hit_limit for w in gameweeks),
            name="gw_hit_lim",
        )
        ...
        if len(options["no_transfer_gws"]) > 0:
            model.add_constraint(
                so.expr_sum(
                    transfer_in[p, w]
                    for p in players
                    for w in options["no_transfer_gws"]
                )
                == 0,
        ...
        model.add_constraints(
            (
                so.expr_sum(
                    squad[p, w]
                    for p in players
                    if player_team[p] == t and merged_data.loc[p, "Pos"] in {"G", "D"}
                )
                <= max_defs_per_team
                for t in teams
                for w in gameweeks
    ...
    if options.get("no_opposing_play") is True:
        gw_opp_teams = {
            ...
            + [(f["away"], f["home"]) for f in fixtures if f["gw"] == w]
            for w in gameweeks
        }
        for gw in gameweeks:
            ...
            if options.get("opposing_play_group", "all") == "all":
                ...
                model.add_constraints(
                    (
                        lineup[p1, gw] + lineup[p2, gw] <= 1
                        for (p1, p2) in opposing_players
                    ),
                    name=f"no_opp_{gw}",
                )
        ...
        cp_penalty = {
            w: opposing_play_penalty
            * so.expr_sum(cp_pen_var[p1, p2, w1] for (p1, p2, w1) in cp_list if w1 == w)
            for w in gameweeks
        }
        model.add_constraints(
            (
                lineup[p1, w] + lineup[p2, w] <= 1 + cp_pen_var[p1, p2, w]
                for (p1, p2, w) in cp_list
            ),
            name="cp1",
        )
        model.add_constraints(
            (cp_pen_var[p1, p2, w] <= lineup[p1, w] for (p1, p2, w) in cp_list),
            name="cp2",
        )
        model.add_constraints(
            (cp_pen_var[p1, p2, w] <= lineup[p2, w] for (p1, p2, w) in cp_list),
            name="cp3",
        )

    if options.get("double_defense_pick") is True:
        team_players = {
            t: [p for p in players if merged_data.loc[p, "name"] == t] for t in teams
        }
        gk_df_players = {
            t: [
                p for p in team_players[t] if player_type[p] == 1 or player_type[p] == 2
            ]
            for t in teams
        }
        weekly_sum = {
            (t, w): so.expr_sum(lineup[p, w] for p in gk_df_players[t])
            for t in teams
            for w in gameweeks
        }
        def_aux = model.add_variables(teams, gameweeks, vartype=so.binary, name="daux")
        model.add_constraints(
            (weekly_sum[t, w] <= 3 * def_aux[t, w] for t in teams for w in gameweeks),
            name="dauxc1",
        )
        model.add_constraints(
            (
                weekly_sum[t, w] >= 2 - 3 * (1 - def_aux[t, w])
                for t in teams
                for w in gameweeks
            ),
            name="dauxc2",
        ...
        for pos, val in price_choices.items():
            ...
            for key, count in value_dict.items():
                target_players = [
                    ...
                    and buy_price[p] >= key - buffer
                    and buy_price[p] <= key + buffer
                ]
                model.add_constraints(
                    (
                        so.expr_sum(squad[p, w] for p in target_players) >= count
                        for w in gameweeks
                    ),
    ...
    if options.get("no_gk_rotation_after") is not None:
        ...
        model.add_constraints(
            (
                lineup[p, w] >= lineup[p, target_gw] - use_fh[w]
                for p in players_gk
                for w in gameweeks
                if w > target_gw
            ),
    ...
    if len(options.get("no_chip_gws", [])) > 0:
        ...
        model.add_constraint(
            so.expr_sum(use_bb[w] + use_wc[w] + use_fh[w] for w in no_chip_gws) == 0,
            name="no_chip_gws",
        )
    ...
    if options.get("only_booked_transfers") is True:
        ...
        model.add_constraints(
            (transfer_in[p, next_gw] == in_players[p] for p in players),
            name="fix_tgw_tr_in",
        )
        model.add_constraints(
            (transfer_out[p, next_gw] == out_players[p] for p in players),
            name="fix_tgw_tr_out",
        )
    ...
    if options.get("no_trs_except_wc", False) is True:
        model.add_constraints(
            (number_of_transfers[w] <= 15 * use_wc[w] for w in gameweeks),
            name="wc_trs_only",
        )
    ...
    if objective == "regular":
        ...
        decay_objective = so.expr_sum(
            gw_total[w] * pow(decay_base, w - next_gw) for w in gameweeks
        )
        model.set_objective(-decay_objective, sense="N", name="total_decay_xp")

    report_decay_base = options.get("report_decay_base", [])
    decay_metrics = {
        i: so.expr_sum(gw_total[w] * pow(i, w - next_gw) for w in gameweeks)
        for i in report_decay_base
    }
    ...
    for iter in range(iteration):
            ...
            with open(sol_file_name, "r") as f:
                ...
                for line in f:
                    ...
                    if words[0] == "Infeasible":
                        raise ValueError(
                            "Infeasible problem instance, check your parameters"
                        )
        ...
        for w in gameweeks:
            for p in players:
                if (
                    squad[p, w].get_value()
                    + squad_fh[p, w].get_value()
                    + transfer_out[p, w].get_value()
                    > 0.5
                ):
                    ...
                    is_squad = (
                        1
                        if (
                            use_fh[w].get_value() < 0.5
                            and squad[p, w].get_value() > 0.5
                        )
                        or (
                            use_fh[w].get_value() > 0.5
                            and squad_fh[p, w].get_value() > 0.5
                        )
                        else 0
                    )
                    ...
                    player_sell_price = (
                        ...
                        if not is_transfer_out
                            ...
                            and transfer_out_first[p, w].get_value() > 0.5
                            else buy_price[p]
                        )
                    )
                    multiplier = (
        ...
        picks_df.sort_values(
            by=["week", "squad", "lineup", "bench", "type"],
        ...
        for w in gameweeks:
            ...
            chip_decision = (
                ...
                + ("FH" if use_fh[w].get_value() > 0.5 else "")
                + ("BB" if use_bb[w].get_value() > 0.5 else "")
                + ("TC" if use_tc_gw[w].get_value() > 0.5 else "")
            )
            if chip_decision != "":
            ...
            lineup_players = picks_df[
                (picks_df["week"] == w) & (picks_df["lineup"] == 1)
            ]
            ...
            summary_of_actions += (
                "Lineup xPts: "
                + str(round(lineup_players["xp_cont"].sum(), 2))
                + "\n---\n\n"
            )
            cumulative_xpts = cumulative_xpts + round(
                lineup_players["xp_cont"].sum(), 2
            )
        ...
        solutions.append(
            {
                ...
                "decay_metrics": {
                    key: value.get_value() for key, value in decay_metrics.items()
                },
            }
        )
        ...
        if iteration_criteria == "this_gw_transfer_in":
            actions = so.expr_sum(
                1 - transfer_in[p, next_gw]
                for p in players
                if transfer_in[p, next_gw].get_value() > 0.5
            ) + so.expr_sum(
                transfer_in[p, next_gw]
                for p in players
                if transfer_in[p, next_gw].get_value() < 0.5
            )
            ...
            actions = so.expr_sum(
                1 - transfer_out[p, next_gw]
                for p in players
                if transfer_out[p, next_gw].get_value() > 0.5
            ) + so.expr_sum(
                transfer_out[p, next_gw]
                for p in players
                if transfer_out[p, next_gw].get_value() < 0.5
            )
            model.add_constraint(actions >= 1, name=f"cutoff_{iter}")
            ...
            actions = (
                so.expr_sum(
                    1 - use_wc[w] for w in gameweeks if use_wc[w].get_value() > 0.5
                )
                + so.expr_sum(
                    use_wc[w] for w in gameweeks if use_wc[w].get_value() < 0.5
                )
                + so.expr_sum(
                    1 - use_bb[w] for w in gameweeks if use_bb[w].get_value() > 0.5
                )
                + so.expr_sum(
                    use_bb[w] for w in gameweeks if use_bb[w].get_value() < 0.5
                )
                + so.expr_sum(
                    1 - use_fh[w] for w in gameweeks if use_fh[w].get_value() > 0.5
                )
                + so.expr_sum(
                    use_fh[w] for w in gameweeks if use_fh[w].get_value() < 0.5
                )
            )
            model.add_constraint(actions >= 1, name=f"cutoff_{iter}")
            ...
            actions = so.expr_sum(
                1 - transfer_in[p, w] for [p, w] in transferred_players
            ) + so.expr_sum(transfer_in[p, w] for [p, w] in remaining_players)
            model.add_constraint(actions >= 1, name=f"cutoff_{iter}")
        elif iteration_criteria == "this_gw_lineup":
            selected_lineup = [
                p for p in players if lineup[p, next_gw].get_value() > 0.5
            ]
            model.add_constraint(
                so.expr_sum(lineup[p, next_gw] for p in selected_lineup)
                <= len(selected_lineup) - iter_diff,
                name=f"cutoff_{iter}",
            )
