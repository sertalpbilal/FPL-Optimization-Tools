...
def create_squad_timeline(current_squad, statistics, picks, filename):
    ...
    am_used = any(
        not df_squad[df_squad['week'] == week]['chip'].isna().all() and
        df_squad[df_squad['week'] == week]['chip'].iloc[0] == 'AM'
    ...
    for gw_idx, week in enumerate(display_weeks):
        if week == base_week:
            ...
            ax.text(gw_idx * gameweek_spacing, base_y + 1, 'Base',
            ...
            if 'chip' in gw_players.columns and not gw_players['chip'].isna().all():
                ...
                if chip == 'AM':
                    ...
                    if not amn_players.empty:
                        manager = amn_players[amn_players['transfer_in'] == 1].iloc[0] if any(
                            amn_players['transfer_in'] == 1) else amn_players.iloc[0]

                        cell = patches.Rectangle(
                            (gw_idx * gameweek_spacing - box_width / 2,
                             am_y_position - box_height / 2),
                        ...
                        if week != base_week:
                            ...
                            if prev_week_int in player_positions and 'AM_MANAGER' in player_positions[prev_week_int] and \
                                    manager['transfer_in'] == 1:
                                ax.plot([
                                    (gw_idx - 1) * gameweek_spacing + box_width / 2,
                                    gw_idx * gameweek_spacing - box_width / 2
                                ], [
                                    player_positions[prev_week_int]['AM_MANAGER'][0],
                                    am_y_position
        ...
        if week != base_week:
            ...
            for pos in ['GKP', 'DEF', 'MID', 'FWD']:
                out_players = [p for p in transfers_out
                               if player_positions[prev_week_int][p][1] == pos]
                in_players = [p for p in transfers_in
                              if player_positions[week_int][p][1] == pos]
                ...
                for out_p, in_p in zip(out_players, in_players):
                    ax.plot([
                        ...
                        player_positions[prev_week_int][out_p][0],
                        player_positions[week_int][in_p][0]
                    ], color=text_color, alpha=0.5, linewidth=1)
