...
def read_sensitivity(options=None):
    ...
    # If all_gws is 'n', use gw and situation or ask for them
        ...
        if situation == "n":
            ...
            plan.loc[:, "banned_next_gw"] = plan["banned_next_gw"].apply(
                ast.literal_eval
            )
            plan.loc[:, "locked_next_gw"] = plan["locked_next_gw"].apply(
                ast.literal_eval
            )

            # Step 2: Explode the lists into rows, so each player ID gets its own row
            banned_df = (
                plan[["id", "banned_next_gw"]].explode("banned_next_gw").dropna()
            )
            locked_df = (
                plan[["id", "locked_next_gw"]].explode("locked_next_gw").dropna()
            )
            ...
            banned_df = (
                banned_df[["player_id"]].drop_duplicates().reset_index(drop=True)
            )
            locked_df = (
                locked_df[["player_id"]].drop_duplicates().reset_index(drop=True)
            )
            ...
            review_data_with_banned["is_banned"] = review_data_with_banned[
                "is_banned"
            ].apply(lambda x: True if x == "both" else False)
            review_data_with_locked["is_locked"] = review_data_with_locked[
                "is_locked"
            ].apply(lambda x: True if x == "both" else False)
            ...
            def print_dataframe(df, title):
                ...
                max_lineup_len = (
                    8  # Lineup column is also percentages, so max length is 6
                )
