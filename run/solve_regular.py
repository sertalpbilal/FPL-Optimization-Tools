...
def solve_regular(runtime_options=None):
    ...
    for result in response:
        iter = result["iter"]
        decay = options.get("decay_base", None)
        ft_value = options.get("ft_value", None)
    ...
    if len(options.get("report_decay_base", [])) > 0:
        try:
            ...
            metrics_df = pd.DataFrame(
                [
                    {"iter": result["iter"], **result["decay_metrics"]}
                    for result in response
                ]
            )
            print(metrics_df)
    ...
    for result in response:
        ...
        for gw in gws:
            ...
            if chip_text != "":
                line_text += "(" + chip_text + ") "
            sell_text = ", ".join(
                picks[(picks["week"] == gw) & (picks["transfer_out"] == 1)][
                    "name"
                ].to_list()
            )
            buy_text = ", ".join(
                picks[(picks["week"] == gw) & (picks["transfer_in"] == 1)][
                    "name"
                ].to_list()
            )
            if sell_text != "":
...
def get_fplteam_link(options, response):
    ...
    if options.get("team_id") is None:
        print(
            "(Do not forget to add your team ID to regular_settings.json file to get a custom link.)"
        )
    ...
    for result in response:
        ...
        for gw in gws:
            lineup_players = ",".join(
                picks[(picks["week"] == gw) & (picks["lineup"] > 0.5)]["id"]
                .astype(str)
                .to_list()
            )
            bench_players = ",".join(
                picks[(picks["week"] == gw) & (picks["bench"] > -0.5)]["id"]
                .astype(str)
                .to_list()
            )
            cap = picks[(picks["week"] == gw) & (picks["captain"] > 0.5)].iloc[0]["id"]
            vcap = picks[(picks["week"] == gw) & (picks["vicecaptain"] > 0.5)].iloc[0][
                "id"
            ]
            chip = picks[picks["week"] == gw].iloc[0]["chip"]
            sold_players = (
                picks[(picks["week"] == gw) & (picks["transfer_out"] > 0.5)]
                .sort_values(by="type")["id"]
                .astype(str)
                .to_list()
            )
            bought_players = (
                picks[(picks["week"] == gw) & (picks["transfer_in"] > 0.5)]
                .sort_values(by="type")["id"]
                .astype(str)
                .to_list()
            )
            ...
            tr_string = ";".join(
                [f"{i},{j}" for (i, j) in zip(sold_players, bought_players)]
            )
            ...
            if gw == 1:
                ...
                prev_lineup = (
                    picks[(picks["week"] == gw - 1) & (picks["lineup"] > 0.5)]
                    .sort_values(by="type")["id"]
                    .astype(str)
                    .to_list()
                )
                now_bench = (
                    picks[(picks["week"] == gw) & (picks["bench"] > -0.5)]
                    .sort_values(by="type")["id"]
                    .astype(str)
                    .to_list()
                )
                ...
                prev_bench = (
                    picks[(picks["week"] == gw - 1) & (picks["bench"] > -0.5)]
                    .sort_values(by="type")["id"]
                    .astype(str)
                    .to_list()
                )
                now_lineup = (
                    picks[(picks["week"] == gw) & (picks["lineup"] > 0.5)]
                    .sort_values(by="type")["id"]
                    .astype(str)
                    .to_list()
                )
                ...
                sub_text = ";".join(
                    [f"{i},{j}" for (i, j) in zip(lineup_to_bench, bench_to_lineup)]
                )
