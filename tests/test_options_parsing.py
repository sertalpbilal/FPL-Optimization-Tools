...
def test_load_multiple_configs(temp_config_files):
    ...
    assert config['export_image'] == True
...
def create_arg_parser():
    ...
    base_parser.add_argument(
        '--config', type=str, help='Path to one or more configuration files (semicolon-delimited)')
    return base_parser
...
def test_cli_multiple_configs():
    ...
    args = parser.parse_known_args(
        ['--config', 'config1.json;config2.json'])[0]
...
def test_config_priority_order(temp_config_files, monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_args = ['script.py', '--config',
                     f'{base_path};{override_path}', '--horizon', '10']
        ...
        cli_options = {k: v for k, v in vars(
        ...
        assert options['export_image'] == True
